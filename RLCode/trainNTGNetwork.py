...
#  learner = DeepQNetwork(sess, scope = 'global', stateSize = env.getStateSize(), actionSize = env.getActionSize(), async = False)
  #learner.loadNetwork('NavToGoalNetwork-3900')
#  explorer = EpsilonGreedyExplorer(possibleActions = possibleActions, continuous = env.isActionContinuous(), epsilon = 1, epsilonDecay = 0.00001, epsilonMin = 0.05)
#  policy = Policy(learner = learner, explorer = explorer)
#  agent = Agent(policy)
#  controller = BatchController(environment = env, agent = agent, batchSize = 300, updateTargetRate = 10)
#
...
#      filename = 'NavToGoalNetwork-' + str(n)
...
from Controller import OrderedController, BatchController
from Environments import GymEnvironment, MazeEnvironment, GridworldEnvironment, AVEnvironment, DoomEnvironment, AvoidBarriersEnvironment, NavigateToGoalEnvironment
from Agents import Agent
from Agents.Policies import Policy
...
envs = [NavigateToGoalEnvironment(maxEpisodeLength = 100) for i in xrange(numAgents)]
controllers = []
  ...
  with sess.as_default(), sess.graph.as_default():
    while not coord.should_stop():
      for n in range(100000):
        r = controller.runOneEpisode()
        print controller.agent.policy.learner.scope + ': ' + str(r) + ', Epsilon: ' + str(controller.agent.policy.explorer.epsilon)
        if n%step == 0 and n > 0:
          filename = 'NTG' + controller.agent.policy.learner.scope + '-Network-' + str(n)
          controller.agent.policy.learner.saveNetwork(filename)
      #coord.request_stop()
...
with tf.Session() as sess:
  ...
  for i in range(numAgents):
    ...
    controller = BatchController(environment = envs[i], agent = copy.copy(agent), batchSize = 300, updateTargetRate = None)
