...
with tf.Session() as sess:
    ...
    explorer = NoisyActionExplorer(possible_actions=possible_actions, continuous=env.is_action_continuous(),
                                     epsilon=epsilon,
    ...
    for n in range(numGames):
        ...
        if n % step == 0 and n > 0:
            if save_network_file_name is not None:
                filename = save_network_file_name + '_actor_' + str(n + previous_number_of_episodes)
                controller.agent.policy.learner.actor_network.save_network(sess, filename)
                filename = save_network_file_name + '_critic_' + str(n + previous_number_of_episodes)
                controller.agent.policy.learner.critic_network.save_network(sess, filename)
